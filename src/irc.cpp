...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #StarCashTEST2\r");
            Send(hSocket, "WHO #StarCashTEST2\r");
        } else {
            // randomly join #StarCash00-#StarCash05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #StarCash%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #StarCash%02d\r", channel_number).c_str());
        }
