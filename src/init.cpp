...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("StarCash version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  StarCashd [options]                     " + "\n" +
                  "  StarCashd [options] <command> [params]  " + _("Send command to -server or StarCashd") + "\n" +
                  "  StarCashd [options] help                " + _("List commands") + "\n" +
                  "  StarCashd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "StarCash:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("StarCash"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("StarCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: StarCash.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: StarCashd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 24426 or testnet: 17778)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 24425 or testnet: 18345)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //if (fTestNet) {
        SoftSetBoolArg("-irc", true); // We want IRC on by default
    //}
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  StarCash is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "StarCash server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("StarCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of StarCash") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart StarCash to complete") << "\n";
