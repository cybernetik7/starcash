StarCash-qt: Qt4 GUI for StarCash
===============================
...
Alternatively, install Qt Creator and open the `StarCash-qt.pro` file.

An executable named `StarCash-qt` will be built.
...
A warning for people using the *static binary* version of StarCash on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of StarCash is linked against libdb5.3.
...
installed causes StarCash-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
